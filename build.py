...
def create_addon(files, browser):
    ...
    for f in files:
        ...
        if f.endswith("manifest.json"):
            ...
            if browser != "firefox":
                del manifest[
                    "applications"
                ]  # Firefox specific, and causes warnings in other browsers...

            if browser == "firefox":
                del manifest["background"][
                    "persistent"
                ]  # Firefox chokes on this, is always persistent anyway

            if browser == "opera":
                manifest["options_ui"][
                    "page"
                ] = "redirector.html"  # Opera opens options in new tab, where the popup would look really ugly
    ...
    if browser == "opera":
        ...
        os.system(
            "./nex-build.sh %s %s %s"
            % (output_file, output_file.replace(".zip", ".nex"), cert)
        )
